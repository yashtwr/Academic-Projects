...
def gg(street_names, lines):
    ...
    for i in range(0, len(vertices)):
        print(i+1, ": ("+str(round(vertices[i][0],2))+','+str(round(vertices[i][1],2))+')')
...
def modify(name, coord, street_names, lines):
    ...
    if name in street_names:
        ...
        print("Error: Street name does not exist, please add the street first to modify it")
    ...
    while True:
        ...
        if (input_line[0:3] == "add" or input_line[0:3] == "mod"):
            if (input_rx.match(input_line[3:])):
                ...
                if(temp[0][1] == " "):
                    print("Error: There should be no spaces before name")
                else:
                ...
                if (re.compile(name_rx).match(input_line[3:])):
                    print("Error: Please enter space between command and street name")
                else:
                    print("Error: Invalid Street name")
            else:
                print("Error: Please enter correct co ordinates, it should be atleast two")
            ...
            if ((re.compile('\s' + name_rx+'$')).match(input_line[2:])):
                ...
                print("Error: Ivalid command, please try again")
        elif (input_line[0:2] == "gg"):
            if re.compile('gg$').match(input_line):
                gg(street_names, lines)
            else:
                print("Error: Invalid gg command")
        else:
            print("Error: Please enter the correct command, ensure it is in lower ")
    sys.exit(0)
