...
```
The simulation has been run for 1000 times. However the first index of the above declared variables would yield the output for part a.
```{r}
for(x in 1:1000)
{
  ...
  if(clinic_time>max(d_time)) # store should close at least at 4pm
  {
    cl<- clinic_time
  }
  else
...
Hence the results has been computed for part a
...
The heat2 column has 0 NA values
```{r}
...
```
Stored only democratic and republic votes
```{r}
...
#Creating empty data-frame to store all the results
...
The following are the observations from the above analysis:
Vermont has the highest percentage of democratic supporters and approx 74% of these votes are from non-married people.
The survey also shows that most of the non-married people are inclined to vote for the democratic party
...
```
The above table shows the first five observations. The above analysis also shows that Vermont has voted the highest for Obama that is the democratic party. It's value is very close to the survey conduced in the previous question data set.

## Q1(D)

### Assumption 1 - Complete Pooling
```{r}
#Creating dummy variable of the output and converting it to numeric type
#gm cannot run when output is a factor
...
penality_factor[1] <- 0 # removing penalty for married:married column
penality_factor[2] <- 0 # removing penalty for married:other column
...
```
Since the assumption is to to have no state level heterogeneity, all the state columns has been heavily penalized by setting very high values of lambda. Since the marital column are not to be penalized, their penalty factor has been set to zero. The model is only able to predict 2.3% of the variations in the data.
...
```
The above model has no pooling and all the states, marital and interaction between then are considered as predictors. It shows an negative relationship between the states and votes for almost all the states. The model is able to show 4.07% of variations in the data set. However, it might be subjected to over fitting.

### Assumption 3 - Partial Pooling
```{r}
penality_factor<- rep(1,ncol(x_2))
#making penalty factor of marital column = 0, since it should not be included 
penality_factor[1] <- 0
penality_factor[2] <-0
set.seed(200)
...
The above model is partially pooled for lasso regression. The best value for lambda is estimated by cv.glmnet() where the type of measure is set to class since it is a classification problem and the penalty factor has been adjusted accordingly. The best value of lambda is then used for prediction. The model is able to show 3.4% of variations in the data .
...
for(i in 1:l)
{
  ...
  pool_graph_df[i,7] <- abs(t5-t7)*100/t2 # partial marital gap/state 
...
```
The above code computes all the values required for plotting graphs for questions
e,f,g. All the computations are stored in data frames which are being used to plot the graphs 
```{r}
...
The above plot shows the vote percentages of the democratic party for partial pooling results. 
The diamond shaped points represents predicted vote percentage
the triangle shaped points represents survey vote percentages
The circle marked points represents actual votes received by the democratic party
The analysis shows that almost all the votes/state lie in the interquartile range and predictions lying close to the original values. 
# Q1(F)
...
The above graph shows the marital gap percentages amongst citizens who voted for democratic party. The marital gap is plotted in percentage for better visualization of the result. The model fails to accurately predict the marriage gap per state when compared to the actual values from the survey. From the prediction results it can be inferred that the lower the marriage gap, the more votes has been received by the democratic party from that state.
...
The above graph shows the voting percentage for democratic party using the no pooling predictions.The predictions have a lot of difference when compared to the actual survey results. Additionally, the partial pooling predictions perform better than the no pooling as the number of outliers are less in the partial pooling graph when compared to no pooling graph.
...
From the graph it can be inferred that marital gap for no pooling predications remain similar to the partial pooling predictions. It can be inferred from the predictions that the lower the marital gap, the higher is the percentage of citizens who voted for the democratic party. 
